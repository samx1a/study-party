...
export function AccountForms({ name, email, timezone }: { name: string; email: string; timezone: string }) {
  ...
  async function deleteAccount(e: React.FormEvent<HTMLFormElement>) {
    ...
    const f = new FormData(e.currentTarget);
    const { error } = await authClient.deleteUser({ password: String(f.get("password")) });
    if (error) {
  ...
  return (
    <div className="mt-8 flex flex-col gap-6">
        ...
        <form onSubmit={saveProfile} className="mt-4 flex flex-col gap-4">
          ...
          <div className="flex flex-col gap-1.5">
            ...
            <p className="text-sm text-muted">
              {timezone}. Your study days follow this. It updates to this device&apos;s zone when you save or join a
              room.
            </p>
          </div>
      ...
      <Card className="border-danger/30">
        ...
        <p className="mt-1 text-sm text-muted">Removes your account, rooms you host, your goals, and your study history.</p>
        <form onSubmit={deleteAccount} className="mt-4 flex flex-col gap-4">
          <div className="flex flex-col gap-1.5">
            ...
            <Input id="del-password" name="password" type="password" required autoComplete="current-password" />
          </div>
