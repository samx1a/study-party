...
export function GoalsPanel({ roomId, meId, goals, onChange, refresh }: Props) {
  ...
  async function add(e: React.FormEvent) {
    ...
    setText("");
    if (
      !(await call(`/api/rooms/${roomId}/goals`, {
        method: "POST",
  ...
  function toggle(g: RoomGoal) {
    ...
    call(`/api/rooms/${roomId}/goals/${g.id}`, {
      method: "PATCH",
  ...
  for (const g of goals) {
    ...
    const entry = others.get(g.userId) ?? {
      name: g.userName,
      goals: [],
      here: false,
    };
  ...
  return (
    <aside
      className="flex w-80 shrink-0 flex-col gap-6 overflow-y-auto rounded-2xl border border-border bg-surface p-4"
      data-testid="goals"
    >
      <section>
