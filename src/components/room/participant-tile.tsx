...
export function ParticipantTile({ participant, screen, camera, goals, size, canRemove, onRemove, onClick }: Props) {
  ...
  return (
    ...
    >
      ...
      <div className="absolute bottom-2 left-2 flex max-w-[70%] items-center gap-1.5 rounded-md bg-black/60 px-2 py-1 text-xs backdrop-blur">
        {micMuted ? <MicOffIcon className="size-3.5 shrink-0 text-muted" /> : <MicIcon className="size-3.5 shrink-0 text-break" />}
...
function Placeholder(props: { small: boolean; tone: "warn" | "muted"; icon: React.ReactNode; children: React.ReactNode }) {
  return (
    <div
      className={cn(
        "flex size-full flex-col items-center justify-center gap-2 bg-bg",
...
function CameraBubble({ camera, name, small, mirror }: { camera?: TrackReference; name: string; small: boolean; mirror: boolean }) {
  return (
    <div
      className={cn(
        "absolute right-2 bottom-2 overflow-hidden rounded-full border-2 border-bg bg-surface-2 shadow-lg",
        small ? "size-10" : "aspect-square w-[20%] min-w-14 max-w-32",
      )}
    >
      <span className="grid size-full place-items-center text-sm font-semibold text-muted">{initials(name)}</span>
...
function CameraVideo({ camera, mirror }: { camera: TrackReference; mirror: boolean }) {
  ...
  if (off) return null;
  return <VideoTrack trackRef={camera} className={cn("absolute inset-0 size-full object-cover", mirror && "-scale-x-100")} />;
}
